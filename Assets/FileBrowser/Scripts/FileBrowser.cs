...
namespace SimpleFileBrowser
{
	public class FileBrowser : MonoBehaviour, IListViewAdapter
	{
			...
			get
			{
				if( m_instance == null )
				{
                    ...
                    if (SceneManager.GetActiveScene().name.Equals("Server"))
                        m_instance = Instantiate<GameObject>(Resources.Load<GameObject>("SimpleFileBrowserCanvasGraphic")).GetComponent<FileBrowser>();
		...
		public Color normalFileColor = new Color32(45, 45, 45, 45);
		public Color hoveredFileColor = new Color32( 90, 90, 90, 45 );
		public Color selectedFileColor = new Color32(60, 60, 60, 45);

        public Color wrongFilenameColor = new Color32( 185, 25, 25, 45 );
										   ...
										   string title = "Save", string saveButtonText = "Save" )
		{
			...
			{
				Debug.LogError( "Multiple dialogs are not allowed." );
				return false;
			}
        ...
        #region Dropdown Adjustments for Oculus
        ...
        IEnumerator DestroyBlocker()
        {
            yield return new WaitForSeconds(1f);

            GameObject blocker = this.gameObject.transform.Find("Blocker").gameObject;
            Destroy(blocker.transform.GetComponent<GraphicRaycaster>());
        }
